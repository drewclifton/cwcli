...
export function openCommand() {
  ...
  withSiteArg(
    cmd
      .description('Open the local site in a browser')
      .option('-p, --port <port>', 'Local port', v => parseInt(v, 10))
      .option('--admin', 'Auto-login and open /wp-admin/')
      .option('--user <user>', 'Username to log in as when using --admin')
    ...
    async ({ siteDir: cwd, opts }) => {
      ...
      // Auto-login flow: ensure mu-plugin, write token options, open wp-admin.
